...
The system was developed as a practical full-stack machine learning application in which model training, backend inference serving, relational data persistence, and web integration are connected into a single working pipeline.
...
Persist prediction results in a relational database
...
Persisting each prediction result in an H2 relational database
Technology Stack
Machine Learning
Python
TensorFlow / Keras
...
CUSTOM_CNN/
├── ML_SERVICES/          # Flask-based ML inference backend and trained model
...
Collecting and organizing acne and clear skin image datasets
Balancing the dataset across classes
Splitting the data into training, validation, and test partitions
Training a custom CNN model using TensorFlow/Keras
...
Confusion matrix
ROC-AUC
Tuning the classification threshold to improve practical prediction behavior
Exporting the final trained model for inference usage
Integrating the final model into the Flask inference service
Model Performance
...
A recent enhancement to the project introduced relational persistence for prediction results.

Each prediction request is now stored in an H2 database through the Spring Boot layer. Each saved record includes:
...
This enhancement improves traceability, demonstrates backend engineering maturity, and aligns the project more closely with production-oriented application design.
...
Score
Decision threshold
Relational persistence of prediction records using H2
Modular separation between ML inference and web application layers
Final trained model included for local execution
...
Once the Spring Boot application is running, the H2 console can be accessed at:

http://localhost:8080/h2-console

Use the following configuration:

JDBC URL: jdbc:h2:file:./data/skinpredictordb
User Name: sa
...
Application Flow
The user uploads a skin image through the Spring Boot interface
Spring Boot sends the uploaded image to the Flask prediction service
Flask preprocesses the image and runs the trained CNN model
Flask returns the prediction response
Spring Boot displays the result to the user
Spring Boot stores the prediction result in the H2 database
Current Limitations
...
The current database setup is intended for local demonstration and development use
Future Improvements
...
Replace local H2 storage with MySQL or PostgreSQL for a more production-oriented database layer
Package both services for cloud deployment
Introduce persistent logging and request tracing
Add a dashboard to view historical prediction records
